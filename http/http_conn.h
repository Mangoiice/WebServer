...
public:
    ...
    int timer_flag; // 标记读写操作是否出错
    int improv;     // 标记这个连接是否已经进行了读写
...
private:
    ...
    /*
    根据process_read解析的报文结果，分析请求报文的类型，比如：FORBIDDEN_REQUEST、BAD_REQUEST之类
    如果是FILE_REQUEST，会将网页文件映射到程序的共享内存中，在write函数中一并发送给客户端
    */ 
...
private:
    ...
    struct iovec m_iv[2];       // io向量机制iovec，用于writev函数同时发送相应报文和服务器网页文件
    int m_iv_count;             // io向量的个数
    int cgi;                    // 是否为POST请求
    char* m_string;             // 存储请求正文数据，只有POST请求需要
    int bytes_to_send;          // 需要发送的字节数
    int bytes_have_send;        // 已经发送的字节数
    char* doc_root;             // 网站根目录

    map<string, string> m_user; // 该连接的用户名和密码
    int m_TRIGMode;             // 为1时socket为ET触发
    int m_close_log;
