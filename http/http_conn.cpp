...
locker m_lock;
map<string,string> users;   // 存储全部用户信息

void http_conn::initmysql_result(connection_pool *connPool) 
{
    // 创建connectionRAII对象，从数据库连接池中取出一个连接
    MYSQL *mysql = NULL;
                    ...
                    int close_log, string user, string passwd, string sqlname) 
{
    m_sockfd = sockfd;
    m_address = addr;
    LOG_INFO("新连接init TRIGMode = %d", TRIGMode);
    m_TRIGMode = TRIGMode;
...
/*
从状态机，用于分析出一行的内容，每一行的结尾标志是\r\n
返回值为行的读取状态，有LINE_OK、LINE_BAD、LINE_OPEN
LINE_OK：接收并处理了完整的一行报文
LINE_BAD：接收到了错误的报文
LINE_OPEN：接收到的报文不完整
*/
...
请求行的结构
GET \thttps://192.168.207.130/localhost \tHTTP/1.1\r\n
*/
http_conn::HTTP_CODE http_conn::parse_request_line(char *text) 
{
    // strpbrk返回第一个在text中出现' '或者'\t'的位置
    m_url = strpbrk(text, " \t");
    if (!m_url) 
    {
        return BAD_REQUEST;
    ...
    */
    ...
    // 此时可以找到url了，会含有http://或者https://，所以要去掉
    *m_version++ = '\0';
    if (strncasecmp(m_url, "http://", 7) == 0) 
    {
        m_url += 7;
        // strchr找到m_url中第一个'/'的位置，用于去掉服务器的ip地址
        m_url = strchr(m_url, '/');
    }
...
http_conn::HTTP_CODE http_conn::parse_content(char *text) 
{
    if (m_read_idx >= (m_content_length + m_checked_idx)) 
    {
        ...
        // POST请求中最后为输入的用户名和密码
        m_string = text;
        // 代表已经解析完了POST请求的请求正文，但是需要返回GET_REQUEST结束循环
        return GET_REQUEST;
    }
    return NO_REQUEST;
}
